#!/bin/bash
#SBATCH --job-name=DLPFC_Inhibitory_neuron_smr_association
#SBATCH --array=0-65 
#SBATCH --nodes=1
#SBATCH --ntasks-per-node=1
#SBATCH --mem=50G #5G # 90G # If running to create parquet files, we need 90Gb if not then 5Gb is sufficient
#SBATCH --time=03:00:00
#SBATCH --output=/dcs04/lieber/hwanglab/Arun/snRNA_aanri/1_source/batch_executables/5_SMR_associations/DLPFC/smr_logs/Inhibitory_neuron_smrAsso_%A_%a.out 
#SBATCH --error=/dcs04/lieber/hwanglab/Arun/snRNA_aanri/1_source/batch_executables/5_SMR_associations/DLPFC/smr_logs/Inhibitory_neuron_smrAsso_%A_%a.err
...
TWAS="/dcs04/lieber/hwanglab/Arun/snRNA_aanri/3_analysis/DLPFC/Inhibitory_neuron/residuals/TWAS"
...
nominalQTL="/dcs04/lieber/hwanglab/Arun/snRNA_aanri/3_analysis/DLPFC/Inhibitory_neuron/residuals/eQTL/DLPFC_Inhibitory_neuron_.lan.tsqtl.nominal.txt"
...
python "/dcs04/lieber/hwanglab/Arun/miRQTL_pipeline/source/software/miRQTL/TWAS/twas_pipeline.py" SMR \
    ...
    --output "$TWAS" \
    --metadata "$metadata" \
    --outPrefix "DLPFC_Inhibitory_neuron" \
    --plinkPath "$plinkPath" \
    --genotype "$genotype" \
    --coordinates "$coords" \
    --gwasDir "$gwas" \
