...
#SBATCH --output=0_step_logs/combineQTLs_%A.out
#SBATCH --error=0_step_logs/combineQTLs_%A.err
#SBATCH --mem=64G
...
celltypes=("Astrocyte" "Excitatory_neuron" "Inhibitory_neuron") 
...
for brainRegion in "${brainRegions[@]}"; do
	...
	# Skip this specific combination
        ...
        if [[ "$brainRegion" == "DLPFC" && "$celltype" == "Astrocyte" ]]; then # This is avoided because of the error detailed below
            echo "Skipping: ${brainRegion} / ${celltype}"
            continue
        fi
        
	if [[ "$brainRegion" == "hippo" && "$celltype" == "Astrocyte" ]]; then # This is avoided because of the error detailed below
            echo "Skipping: ${brainRegion} / ${celltype}"
            continue
        fi
